...
function updateAnimatedCoinsDisplay() {
  ...
  if (displayCoins === targetCoins) {
    headerElem.textContent = targetCoins.toLocaleString();
    return;
  }
  ...
  coinsTickTimer = setInterval(() => {
    ...
    headerElem.textContent = displayCoins.toLocaleString();
    audio.playSpinTick();
...
function forceCoinsUpdateImmediate() {
  displayCoins = appState.profile.totalCoins;
  document.getElementById('headerCoinCount').textContent = displayCoins.toLocaleString();
}
...
function processSequenceSpinResult(symbols) {
  ...
  if (winAmount > 0) {
    ...
    audio.playWinTwin();
    confettiEngine.addExplosion(null, null, 15, 'mixed');
  } else {
    if (feedText) {
...
function finishRunningSession() {
  ...
  if (currentSessionWins > 0) {
    ...
    confettiEngine.addExplosion(null, null, currentSessionWins > 200 ? 70 : 35, 'mixed');
...
function triggerWinEpicPopup(title, description, amount, type) {
  ...
  valueTxt.textContent = `+${amount}`;

  if (type === 'jackpot') {
    titleTxt.className = 'win-combo-title uppercase tracking-widest text-[#fbbf24]';
    audio.playWinJackpot();
  } else {
    titleTxt.className = 'win-combo-title uppercase tracking-widest text-[#00f2fe]';
    audio.playWinTwin();
  }
...
function tickDailyCountdown(msRemaining) {
  ...
  const updateDisplay = () => {
    ...
    const pad = n => String(n).padStart(2, '0');
    timerText.textContent = `${pad(hrs)}:${pad(mins)}:${pad(secs)}`;
    tempMs -= 1000;
  };
...
function executeDailyRewardClaim() {
  ...
  confettiEngine.addExplosion(null, null, 40, 'gold_only');
...
function updateProfileDashboard() {
  ...
  document.getElementById('profileJoinDate').textContent = appState.profile.joinDate || "Juni 2026";

  // Counters
  document.getElementById('statTotalSpins').textContent = appState.profile.totalSpins;
  document.getElementById('statJackpots').textContent = appState.profile.jackpotsWon;
  document.getElementById('statTwins').textContent = appState.profile.twinsWon;
...
function prepareProfileFormModal() {
  ...
  // Load current values
  document.getElementById('inputUsername').value = appState.profile.username;
  document.getElementById('inputBio').value = appState.profile.bio;
...
function handleCustomImageUpload(file) {
  ...
  reader.onload = function(e) {
    ...
    document.getElementById('fileUploadStatus').textContent = "Gambar kustom dimuat ✅";
...
document.addEventListener('DOMContentLoaded', () => {
  ...
  document.getElementById('winPopupDismissBtn').addEventListener('click', () => {
    dismissWinPopup();
  });
  ...
  document.getElementById('dailyChest').addEventListener('click', () => {
    executeDailyRewardClaim();
  });
...
function simulateLoadingProgress() {
  ...
  const interval = setInterval(() => {
    ...
    if (currentPercent >= 100) {
      ...
      prctText.textContent = '100%';
      statusText.textContent = statusLabels[4];
      ...
      prctText.textContent = `${currentPercent}%`;
      
      const idx = Math.min(statusLabels.length - 2, Math.floor(currentPercent / 25));
      statusText.textContent = statusLabels[idx];
    }
  }, 50);
}
